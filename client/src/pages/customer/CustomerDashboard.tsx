...
export default function CustomerDashboard() {
  ...
  const [myActiveBookings, setMyActiveBookings] = useState<Booking[]>([]);
    ...
    (async () => {
      try {
        const [vRes, bRes] = await Promise.all([
          api.listVehicles({ page: 1, limit: 12, sortBy: 'popular' }),
          api.getMyBookings({ customerId: user?.id, page: 1, limit: 10 }),
        ]);
        setVehicles(vRes.data);
        const active = bRes.data.filter(b => b.status === 'active');
        const upcoming = bRes.data.filter(b => b.status === 'confirmed');
        setMyActiveBookings(active);
        setMyUpcomingBookings(upcoming);
      } catch (e) {
        // swallow errors for dashboard demo
      }
    })();
  }, [user?.id]);
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid md:grid-cols-3 gap-6">
              {vehicles.filter(v => v.status === 'available').slice(0, 3).map((vehicle) => (
                <motion.div
                  key={vehicle.id}
                  whileHover={{ y: -8 }}
